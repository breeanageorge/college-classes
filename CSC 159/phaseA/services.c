...
void ForkService(int *ebx_p){
   ...
   *ebx_p = pid;
   ...
   MyMemcpy((char *)&signal_table[pid][0], (char *)&signal_table[run_pid][0], sizeof(func_p_t [SIG_NUM]));
}
...
void ExitService(int exit_code){
   ...
   if(pcb[ppid].state != WAITCHILD){
      ...
      pcb[run_pid].trapframe_p->ebx = (unsigned int)exit_code;
      run_pid = -1;
      if(signal_table[ppid][SIGCHILD]){
   ...
   *p = exit_code;
   ...
   set_cr3(OS_TT);
   for(i = 0; i < 5; i++){
      page_p = (char *)(PAGE_BASE + (pcb[run_pid].page[i] * PAGE_SIZE));
...
void WaitchildService(int *exit_code_p, int *child_pid_p){
   ...
   *exit_code_p = (int)(pcb[child_pid].trapframe_p->ebx);
   set_cr3(ORIG_TT);
   ...
   set_cr3(OS_TT);
   for(i = 0; i < 5; i++){
      page_p = (char *)(PAGE_BASE + (pcb[child_pid].page[i] * PAGE_SIZE));
...
void ExecService(func_p_t p, int arg){
   int page, entry, i, trapframe_location, *q;
   char *page_p;
   int zero = 0;
   ...
   //TT
   pcb[run_pid].TT = PAGE_BASE + (pcb[run_pid].page[0] * PAGE_SIZE);
   ...
   page_p = (char *)(PAGE_BASE + (pcb[run_pid].page[1] * PAGE_SIZE));
   q[entry] = (int)page_p | 0x003;

   entry = (VM_END & 0xffc00000) >> 22;
   page_p = (char *)(PAGE_BASE + (pcb[run_pid].page[2] * PAGE_SIZE));
   q[entry] = (int)page_p | 0x003;
   ...
   page_p = (char *)(PAGE_BASE + (pcb[run_pid].page[3] * PAGE_SIZE));
   q[entry] = (int)page_p | 0x003;
   ...
   page_p = (char *)(PAGE_BASE + (pcb[run_pid].page[4] * PAGE_SIZE));
   q[entry] = (int)page_p | 0x003;
   ...
   pcb[run_pid].trapframe_p->eip = VM_START;
